...
?> 
 ...
 <table width="100%" border="0" class="table table-striped table-bordered table-hover table-condensed" id="example1">
     <thead>
        <tr>
          ...
          <th width="10%" class="text-center">Montant salaire percu<br><input type="text" class="form-controls input-sm" placeholder="" data-index="1" size="20" /> </th>
           <th width="10%" class="text-center">Trop percu<br><input type="text" class="form-controls input-sm" placeholder="" data-index="1" size="20" /> </th>
 ...
 foreach ($liste_regularisation as $key => $value) {
   ?>
        <tr>
          <td> <?php echo $value->nom.' '.$value->prenom ?></td>
          <td><?php echo $value->montant_salaire_percu ?></td>
          <td><?php echo $value->montant_trop_percu ?></td>
          <td><?php echo date("d-m-Y",strtotime($value->date_regularisation)) ?></td>  
           <td width="15%" class="text-center"> <a href="#" class="btn btn-danger btn-sm "  data-toggle="modal" data-target="#del<?php echo $value->idpersonnel_regularisation ?>" ><span class="fa fa-trash"></span> supprimer </a></td>
      

          
                      <div class="modal fade" id="del<?php echo $value->idpersonnel_regularisation ?>" tabindex="-1" role="dialog" aria-labelledby="Fenêtre de confirmation" aria-hidden="true">
                        <div class="modal-dialog modal-md">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h4><i class="fa fa-trash"></i> Suppresion d'une régularisation de: <?= $value->nom.' '.$value->prenom; ?></h4>
                            </div>
                            <form action="" method="post">
                            <div class="modal-body">
                                <input type="hidden" value="<?php echo $value->idpersonnel_regularisation;  ?>" name="idpersonnel_regularisation">                             
